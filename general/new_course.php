...
{	
        ...
        $where	    =	" course_id =". $_GET['course_id'];			
				$tbl				=   "tbl_courses";				
				$insert_id	= $classSave->update_record($tbl, $data, $where, 0);				
				if($insert_id)
				{
					$query			      =	$classSave->update_record($tbl, $data, $where, 1);
					$data             = $_POST;						
					$tbl_logs         = "tbl_logs";
					$logs_fields			=	array("user_id", "query");
				  $logs_static_fields		=	" , addedon='". $time ."', status='1'";
					$data['query']	  =	$query;
					$data['user_id']  = $_SESSION['ins_id'];
...
{	     
        ...
        $where				=	" course_id='". $_GET['course_id'] ."'";	
				$tbl				  = "tbl_courses";                			
				$insert_id			=	$classSave->update($tbl,$fields,$static_fields,$data,$where,0);			
				if($insert_id)
				{
					$query			      =	$classSave->update($tbl,$fields,$static_fields,$data,$where,1);	
					$tbl_logs         = "tbl_logs";
					$logs_fields			=	array("user_id", "query");
				  $logs_static_fields		=	" , addedon='". $time ."', status='1'";
					$data['query']	  =	$query;
...
{	
				...
				$data				=	$_POST;			
				$fields				=	array("user_id", "course_code", "course_name");
				$static_fields		=	" , course_addedon='". $time ."', course_status='1'";				
				$tbl				=       "tbl_courses";			 
